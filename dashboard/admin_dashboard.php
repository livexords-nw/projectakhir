...
// Query total harga
$total_harga_result = $connection->query("SELECT SUM(harga) AS total FROM produk");
if ($total_harga_result && $total_harga_result->num_rows > 0) {
    $total_harga = $total_harga_result->fetch_assoc()['total'];
}
...
<script>
  const produkTerjualChart = new Chart(document.getElementById('produkTerjualChart'), {
    ...
    options: {
      ...
      plugins: {
        legend: { position: 'top' },
        title: { display: true, text: 'Grafik Penjualan Produk' }
      },
      scales: {
        y: { beginAtZero: true }
      }
